<?php
	...
	/*************************** start memebership new order create *********/
		...
		$dataLead = array(
					...
					$respLead = $konnektive->import_lead($dataLead);
					
					$lead = json_decode($respLead);
				/* 	echo '<pre>';
					print_r($lead); 
					echo '</pre>'; */
					...
					if ($lead->result === "SUCCESS") {
						...
						$memShipProdId = 135;
						$memberReqData = $data;
						$memberReqData["campaign_id"] = $memShipCampId;
						$memberReqData["product1_id"] = $memShipProdId;
						$memberReqData["orderId"] = $lead->message->orderId;
						
						$respMembProd = $konnektive->import_order($memberReqData);
						$respMembProd = json_decode($respMembProd);
						
						if ($respMembProd->result === "ERROR") {
							$status = 'DECLINED';
							$price        = 0.00;
						}else{
							$status       = $respMembProd->message->orderStatus;
							$price        = $respMembProd->message->amountPaid;
						}
						...
						$postData = array(
										...
										'ip_address'   => $ip_address,
										'aff_id' 	   => $aff_id,
										'order_id'     => $memberReqData["orderId"],
										'cvv_data'     => encrypt(json_encode($sync)),	
		...
		/********************** end ***************/
		// =============== Store Data In Memeber Ship Porta =====================
		...
		$memberData = array(
			...
			'order_id' => $resp->message->orderId,
			'offer_url' => 'localhost'
			);
			$_SESSION['member_data'] = $memberData;
